...
extern "C" 
{
	...
	body coordinate system system is always X - positive forward ,
											...
											Z - positive to right 
	*/
	M21L_API void ed_fm_add_local_force(double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);
	M21L_API void ed_fm_add_global_force(double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);
	...
	*/
	M21L_API bool ed_fm_add_local_force_component(double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);
	M21L_API bool ed_fm_add_global_force_component(double & x,double &y,double &z,double & pos_x,double & pos_y,double & pos_z);

	M21L_API void ed_fm_add_global_moment(double & x,double &y,double &z);
	M21L_API void ed_fm_add_local_moment(double & x,double &y,double &z);
	...
	*/
	M21L_API bool ed_fm_add_local_moment_component(double & x,double &y,double &z);
	
	M21L_API bool ed_fm_add_global_moment_component(double & x,double &y,double &z);

	M21L_API void ed_fm_simulate(double dt);

	M21L_API void ed_fm_set_atmosphere(double h,//altitude above sea level
	...
	*/
	M21L_API void ed_fm_set_current_mass_state (double mass,
	...
	called before simulation to set up your environment for the next step
	*/
	M21L_API void ed_fm_set_current_state (double ax,//linear acceleration component in world coordinate system
	...
	*/
	M21L_API void ed_fm_set_current_state_body_axis(double ax,//linear acceleration component in body coordinate system
	...
	input handling
	*/
	M21L_API void ed_fm_set_command (int command,
							 float value);
	/*
	 Mass handling 
	...
	*/
	M21L_API bool ed_fm_change_mass  (double & delta_mass,
	...
	*/
	M21L_API void   ed_fm_set_internal_fuel(double fuel);
	/*
		get internal fuel volume 
	*/
	M21L_API double ed_fm_get_internal_fuel();
	/*
		set external fuel volume for each payload station , called for weapon init and on reload
	*/
	M21L_API void  ed_fm_set_external_fuel (int	 station,
														double fuel,
														double x,
														double y,
														double z);
	...
	*/
	M21L_API double ed_fm_get_external_fuel ();
	
	M21L_API void ed_fm_set_draw_args (EdDrawArgument * drawargs,size_t size);
	M21L_API void ed_fm_configure		(const char * cfg_path);


	M21L_API double ed_fm_get_param(unsigned index);

	/*
	call backs for diffrenrt starting conditions
	*/
	M21L_API void ed_fm_cold_start();
	M21L_API void ed_fm_hot_start();
	M21L_API void ed_fm_hot_start_in_air();
